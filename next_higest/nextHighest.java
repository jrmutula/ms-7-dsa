...
public class nexthighest {
    private static int findNextHighest(int num) {
        ...
        if (rightIndex == -1)
            throw new RuntimeException(
                    "Not possible to make next greater number. "
                            + "Input number is the greatest "
                            + "number that can be formed from the digits.");
    ...
    public static void main(String[] args) {
        ...
        try {
            int nextLargest = findNextHighest(number);
            System.out.println("Next highest number is: " + nextLargest);
